...
function submitdetails(event) {
    ...
    axios
        .post("https://crudcrud.com/api/6eba019762ed47fb84de6d7ebddffc6f/admindata", obj)
        .then((response) => {
            showuseronscreen(response.data)
            console.log(response)
        })
...
window.addEventListener("DOMContentLoaded", () => {
    axios
        .get("https://crudcrud.com/api/6eba019762ed47fb84de6d7ebddffc6f/admindata")
        .then((response) => {
            for (let i = 0; i < response.data.length; i++) {
                showuseronscreen(response.data[i])
            }
...
function showuseronscreen(obj) {
    ...
    del.onclick = function() {
        axios
            .delete(`https://crudcrud.com/api/6eba019762ed47fb84de6d7ebddffc6f/admindata ${obj._id}`)
            .then((response) => {
                parentelem.removeChild(childelem)
            })
            .catch((error) => {
